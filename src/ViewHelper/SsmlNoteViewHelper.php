			...
			foreach($context->getConfig('note/export'.(($category) ? '/'.$category : ''))['properties'] as $propertyId => $column) {
				...
				if ($property) {
					...
					elseif ($propertyId == 'average') {
						...
						if (isset($view->averages[$key]) && $view->averages[$key]['reference_value'] && array_key_exists($key, $view->averages)) $average = $view->averages[$key]['sum'] / $view->averages[$key]['reference_value'] * $context->getConfig('student/parameter/average_computation')['reference_value'];
