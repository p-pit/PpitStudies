	...
	public function repriseAction()
	{
		...
		foreach ($cursor as $noteLink) {
			if ($noteLink->class != $noteLink->account_class) {
				...
				echo $noteLink->id . ';' . $noteLink->note_id . ';' . $noteLink->place_id . ';' . $noteLink->account_id . ';' . $noteLink->name . ';' . $noteLink->school_period . ';' . $noteLink->subject . ';' . $noteLink->assessment . ';' . "\n";
			}
		}
		return $this->response;
		...
		foreach ($noteLinks as $noteLink) {
			...
			if (count($nUplets > 1)) {
				foreach ($nUplets as $row) {
					echo $row->id . ';' . $row->note_id . ';' . $row->place_id . ';' . $row->account_id . ';' . $row->name . ';' . $row->school_period . ';' . $row->subject . ';' . $row->assessment . ';' . "\n";
				}
				echo "\n";
			}
		}
